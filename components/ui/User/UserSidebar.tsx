...
export default function Sidebar() {
  ...
  const main: NavItem[] = useMemo(
    () => [
      { name: "Dashboard", link: "/userdashboard", icon: <MdDashboard /> },
      { name: "Attendence", link: "/userattendance", icon: <FiUsers /> },
      { name: "Tasks", link: "/usertasks", icon: <FaUserCircle /> },
    ...
    return (
      <Link
        ...
        className={[
          ...
          active
            ? "bg-[#4F6CF7]/10 text-[#2D49D7] dark:text-[#6B8AFF] dark:bg-[#4F6CF7]/20 shadow-[0_0_0_4px_rgba(79,108,247,0.10)] dark:shadow-[0_0_0_4px_rgba(79,108,247,0.20)]"
      ...
      >
        <span
          className={[
            "text-xl",
            active ? "text-[#2D49D7] dark:text-[#6B8AFF]" : "text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300",
          ].join(" ")}
        >
          {item.icon}
        </span>
  ...
  return (
    <aside
      className={[
        "sticky top-0 h-screen shrink-0 bg-white dark:bg-slate-800",
        "border-r border-[#E5E9F5] dark:border-slate-700",
